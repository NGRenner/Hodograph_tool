...
def plot_combined(ds, file_path, lat, lon, model, output_file=None):
    ...
    Creates a combined figure with three panels:
      - Left panel (spanning two rows): Skew-T/Log-P diagram.
      - Top-right panel: Hodograph (with BRM/BLM, height markers, etc.).
      - Bottom-right panel: Calculated variables (CAPE, CIN, LCL, 0-3 km SRH, critical angle).
    Data is extracted from the pre-sliced dataset ds.
    """
    ...
    # Left column (spanning both rows) = Skew-T.
    # Right column: top cell = Hodograph; bottom cell = calculated variables.
    fig = plt.figure(figsize=(14, 8))
    gs = gridspec.GridSpec(nrows=2, ncols=2, width_ratios=[2, 1], height_ratios=[2, 1],
                            wspace=0.3, hspace=0.3)
    ax_skew = fig.add_subplot(gs[:, 0])   # All rows in left column for Skew-T.
    ax_hodo = fig.add_subplot(gs[0, 1])    # Top-right cell for hodograph.
    ax_calc = fig.add_subplot(gs[1, 1])    # Bottom-right cell for calculations.
    ax_calc.axis("off")  # Hide axis lines on the text panel

    # ===== Skew-T/Log-P Plotting on Left Panel =====
    # Define pressure levels (hPa) for interpolation.
    ...
    # Extract variables for the Skew-T (t and r; and also u, v, gh for barbs and geopotential).
    t = ds['t']    # Temperature in Kelvin.
    r = ds['r']    # Relative Humidity in %.
    
    t_interp, r_interp, u_interp_skew, v_interp_skew, z_interp_skew, levels_used_skew = [], [], [], [], [], []
    for lev in tqdm(levels, desc="Interpolating SkewT Data"):
        ...
        r_level = r.isel(isobaricInhPa=idx).values
        u_level = ds['u'].isel(isobaricInhPa=idx).values
        v_level = ds['v'].isel(isobaricInhPa=idx).values
        z_level = ds['gh'].isel(isobaricInhPa=idx).values
        ...
        r_pt = griddata((lat_grid.ravel(), lon_grid.ravel()),
                        r_level.ravel(), (lat, lon), method='nearest')
    ...
    levels_used_skew = np.array(levels_used_skew)
    
    # Create profiles and sorted arrays.
    p_profile = levels_used_skew * units.hPa
    ...
    prof = parcel_profile(p_sorted_skew, T_sfc,Td_sfc) if False else parcel_profile(p_sorted_skew, T_sfc, Td_sfc)
    ...
    # ===== Hodograph Plotting on Top-Right Panel =====
    # Use your original hodograph code so that all markers and labels are preserved.
    u = ds['u']
    v = ds['v']
    z = ds['gh']
    ...
    # Create a text block with the key calculated values.
    calc_text = (
